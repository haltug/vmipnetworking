...
const simsignalwrap_t BaseMobility::catHostStateSignal = simsignalwrap_t(MIXIM_SIGNAL_HOSTSTATE_NAME);
...
{
    if (stage == 0){
        BaseMobility::findHost()->subscribe(catHostStateSignal, this);
...
void BaseMobility::reflectIfOutside(BorderHandling wo, inet::Coord& stepTarget,
...
BaseMobility::BorderHandling BaseMobility::checkIfOutside( inet::Coord targetPos,
...
bool BaseMobility::handleIfOutside(BorderPolicy policy, inet::Coord& stepTarget,
								   inet::Coord& targetPos, inet::Coord& step,
								   double& angle)
{
...
cModule *const BaseMobility::findHost(void)
{
    return FindModule<>::findHost(this);
}

const cModule *const BaseMobility::findHost(void) const
{
    return FindModule<>::findHost(this);
}

void BaseMobility::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj) {
    Enter_Method_Silent();
...
void BaseMobility::handleHostState(const HostState& state)
{
