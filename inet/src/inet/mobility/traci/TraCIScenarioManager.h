...
//		virtual int numInitStages() const { return std::max(cSimpleModule::numInitStages(), 2); }
	...
	protected:
		...
		void addModule(std::string nodeId, std::string type, std::string name, std::string displayString, const inet::Coord& position, std::string road_id = "", double speed = -1, double angle = -1);
