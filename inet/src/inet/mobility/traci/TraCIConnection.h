...
class TraCIConnection
{
	public:
		 ...
		 */
		inet::Coord traci2omnet(TraCICoord coord) const;
		std::list<inet::Coord> traci2omnet(const std::list<TraCICoord>&) const;

		/**
		 * convert OMNeT++ coordinates to TraCI coordinates
		 */
		TraCICoord omnet2traci(inet::Coord coord) const;
		std::list<TraCICoord> omnet2traci(const std::list<inet::Coord>&) const;
