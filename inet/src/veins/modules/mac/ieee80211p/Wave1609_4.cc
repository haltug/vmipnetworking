...
#include "veins/modules/mac/ieee80211p/Wave1609_4.h"
...
Define_Module(Wave1609_4);

void Wave1609_4::initialize(int stage)
{
...
void Wave1609_4::handleLowerControl(cMessage* msg) {
    ...
    if (msg->getKind() == MacToPhyInterface::TX_OVER) {
        ...
        cRuntimeError("Wave1609_4 received control message from lower layer that has unknown type.");
    }
...
void Wave1609_4::setActiveChannel(t_channel state) {
    activeChannel = state;
    assert(state == type_CCH || (useSCH && state == type_SCH));
}

void Wave1609_4::finish() {
    //clean up queues.
...
Signal* Wave1609_4::createSignal(simtime_t start, simtime_t length, double power, uint64_t bitrate, double frequency) {
...
bool Wave1609_4::guardActive() const {
...
simtime_t Wave1609_4::timeLeftTillGuardOver() const {
    ASSERT(useSCH);
...
simtime_t Wave1609_4::timeLeftInSlot() const {
    ASSERT(useSCH);
...
void Wave1609_4::changeServiceChannel(int cN) {
    ASSERT(useSCH);
...
void Wave1609_4::setTxPower(double txPower_mW) {
    txPower = txPower_mW;
}
void Wave1609_4::setMCS(enum PHY_MCS mcs) {
    ASSERT2(mcs != MCS_DEFAULT, "invalid MCS selected");
    bitrate = getOfdmDatarate(mcs, BW_OFDM_10_MHZ);
    setParametersForBitrate(bitrate);
}

void Wave1609_4::setCCAThreshold(double ccaThreshold_dBm) {
...
int Wave1609_4::EDCA::createQueue(int aifsn, int cwMin, int cwMax,t_access_category ac) {
...
Wave1609_4::t_access_category Wave1609_4::mapPriority(int prio) {
...
void Wave1609_4::EDCA::backoff(t_access_category ac) {
...
void Wave1609_4::EDCA::revokeTxOPs() {
    for (std::map<t_access_category, EDCAQueue>::iterator iter = myQueues.begin(); iter != myQueues.end(); iter++) {
...
void Wave1609_4::channelBusySelf(bool generateTxOp) {
...
void Wave1609_4::channelIdle(bool afterSwitch) {
...
void Wave1609_4::setParametersForBitrate(uint64_t bitrate) {
...
simtime_t Wave1609_4::getFrameDuration(int payloadLengthBits, enum PHY_MCS mcs) const {
    simtime_t duration;
    if (mcs == MCS_DEFAULT) {
