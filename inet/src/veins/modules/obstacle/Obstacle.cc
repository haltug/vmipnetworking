...
void Obstacle::setShape(Coords shape) {
	coords = shape;
	bboxP1 = inet::Coord(1e7, 1e7);
	bboxP2 = inet::Coord(-1e7, -1e7);
...
const inet::Coord Obstacle::getBboxP1() const {
	return bboxP1;
}

const inet::Coord Obstacle::getBboxP2() const {
	return bboxP2;
}
...
namespace {
	...
	bool isPointInObstacle(inet::Coord point, const Obstacle& o) {
		bool isInside = false;
	...
	double segmentsIntersectAt(inet::Coord p1From, inet::Coord p1To, inet::Coord p2From, inet::Coord p2To) {
		inet::Coord p1Vec = p1To - p1From;
		inet::Coord p2Vec = p2To - p2From;
		inet::Coord p1p2 = p1From - p2From;
...
double Obstacle::calculateAttenuation(const inet::Coord& senderPos, const inet::Coord& receiverPos) const {
