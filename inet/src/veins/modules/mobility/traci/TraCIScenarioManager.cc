...
using inet::TraCIBuffer;
using inet::TraCICoord;

Define_Module(inet::TraCIScenarioManager);
	...
	if (obstacles) {
		{
			...
			for (std::list<std::string>::iterator i = ids.begin(); i != ids.end(); ++i) {
				...
				if (typeId == "building") {
					std::list<inet::Coord> coords = getCommandInterface()->polygon(id).getShape();
					Obstacle obs(id, 9, .4); // each building gets attenuation of 9 dB per wall, 0.4 dB per meter
					std::vector<inet::Coord> shape;
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf) {
	...
	if (!mod) {
		...
		for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++) {
			...
			EV << "processVehicleSubscription called." << endl;
