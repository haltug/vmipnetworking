...
class TraCICommandInterface
{
	public:
		...
		std::pair<double, double> getLonLat(const inet::Coord&);
		double getDistance(const inet::Coord& position1, const inet::Coord& position2, bool returnDrivingDistance);
		...
		void addPolygon(std::string polyId, std::string polyType, const TraCIColor& color, bool filled, int32_t layer, const std::list<inet::Coord>& points);
		class Polygon {
			public:
				...
				std::string getTypeId();
				std::list<inet::Coord> getShape();
				void setShape(const std::list<inet::Coord>& points);
		...
		// Poi methods
		void addPoi(std::string poiId, std::string poiType, const TraCIColor& color, int32_t layer, const inet::Coord& pos);
		class Poi {
			public:
		...
		class GuiView {
			public:
				...
				void setBoundary(inet::Coord p1, inet::Coord p2);
	...
	private:
		...
		inet::Coord genericGetCoord(uint8_t commandId, std::string objectId, uint8_t variableId, uint8_t responseId);
		...
		std::list<inet::Coord> genericGetCoordList(uint8_t commandId, std::string objectId, uint8_t variableId, uint8_t responseId);
};
